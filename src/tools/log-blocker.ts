...
export async function logBlockerHandler(args: z.infer<typeof logBlockerSchema>) {
  ...
  if (!res.success) {
    return { content: [{ type: "text" as const, text: `Error logging blocker: ${res.message}` }] };
  }

  return { content: [{ type: "text" as const, text: `Blocker logged: ${args.description}` }] };
}
