...
export async function recallWorkHandler(args: z.infer<typeof recallWorkSchema>) {
  ...
  const res = await apiRequest(
    "GET",
    `/progress/${encodeURIComponent(projectId)}/search?${params.toString()}`
  );

  if (!res.success) {
    return { content: [{ type: "text" as const, text: `Error searching work history: ${res.message}` }] };
  }
